...
export class Opt<T = unknown> extends Variable<T> {
  ...
  init(cacheFile: string) {
    const cacheVal = JSON.parse(Utils.readFile(cacheFile) || "{}")[this.id]
    if (cacheVal !== undefined) this.value = cacheVal
...
const getOptions = (object: object, path = ""): Opt[] => Object.keys(object).flatMap(key => {
